...
public class FileDownloader
{
	...
	public FileDownloader(String taskId, Context context, String downloadUrl,
	...
	public int download(DownloadCallbacks callbacks) throws Exception
	{
		try
		{
			callbacks.onDownloadStart(taskId, getFileSize());
			...
			while (notFinish)
			{
				...
				if (callbacks != null){
					callbacks.onDownloadProgress(taskId, this.downloadSize);
				}
			}
			if (downloadSize == this.fileSize){
				if (callbacks != null)
				{
					callbacks.onDownloadFinish(taskId);
				}
			fileService.delete(this.downloadUrl);
			}
		}
		catch (Exception e)
		{
			...
			if (callbacks != null)
			{
				callbacks.onDownloadException(taskId, e);
			}
			fileService.delete(this.downloadUrl);
			suspend();
		}
